...
- 除最后一个块外，每个块必须恰好为 524288 字节。
- 最后一个块长度范围是 1 至 524288 字节。
- 不为对齐文件追加长度为 0 的块。
- 空源文件不产生任何块。
...
```text
(n - 1) × 524288 < source_size <= n × 524288
```
...
2. 从源流累计读取最多 524288 字节。上游流的单次 chunk 大小不构成块边界。
...
2. 从源输入累计读取最多 524288 字节作为对应块。
3. 若仍有期望摘要但源输入已经没有字节，返回 `SOURCE_TOO_SHORT`。
4. 计算实际块摘要并比较；不匹配时返回块编号、源偏移、期望值和实际值。
5. 若实际块不足 524288 字节，则它必须是最后一块；种子文件中还有摘要时返回 `SOURCE_TOO_SHORT`。
...
```text
source_offset = i × 524288
seed_offset   = i × 32
```

读取期望摘要后，对调用方提供的块字节计算 SHA-256 并比较。单块接口不得自行假定不足 524288 字节的块一定是最后一个块；这一事实需要块数量或源文件长度作为额外上下文。
...
即每 16 MiB 完整源数据对应约 1 KiB 种子数据，不计最后块取整。
...
source_size    = 524288
block_count    = 1
block_hash_hex = 07854d2fef297a06ba81685e660c332de36d5d18d546927d30daad6d7fda1541
seed_size      = 32
seed_hash_hex  = dd0c9a9888ecf7fba4d607be64fa7003a628ed8980ac9f08d76d6a0da17e0dc2
```

### 15.4 必测边界与失败用例

- 1 字节、524287 字节、524288 字节、524289 字节；
- 两个相同块，验证重复摘要仍按顺序保留；
